...
<html xml:lang="zh-CN" xmlns="http://www.w3.org/1999/xhtml" lang="zh-CN"><head>
  ...
  <title>已发送留言 <?php echo $this->session->userdata('uname');?>的博客 - SYSIT个人博客</title>
...
<ul class="tabnav"> 
	<li class="tab1"><a href="Blog/inbox">所有留言<em>(0)</em></a></li>
	<li class="tab4 current"><a href="Blog/outbox">已发送留言<em>(<?php echo $receive_count->count;?>)</em></a></li>
</ul>
<div class="MsgList">
<ul>
    <?php
        foreach ($send as $val) {
            ...
            <li id="msg_186774">
                <span class="sender">
                    <a href="Blog/index" target="user">
                        <img src="images/portrait.gif"
                             alt="<?php echo $this->session->userdata('uname');?>"
                             title="<?php echo $this->session->userdata('uname');?>" class="SmallPortrait"
                             user="154741" align="absmiddle">
                    </a>
                </span>
                <span class="msg">
			<div class="outline">
				<a href="Blog/index" target="user"><?php echo $this->session->userdata('uname');?></a>
				发送于
                <?php
                ...
                ?>前 (<?php echo $val->ADD_TIME;?>)
				<a href="javascript:delete_out_msg(186774)" id="<?php echo $val->SENDER;?>">删除</a>
			</div>
			<div class="content"><div class="c"><?php echo $val->CONTENT;?></div></div>
		</span>
                <div class="clear"></div>
            </li>
            <?php
...
<script>
    $('.outline a:last-child').click(function (e) {
        $(this).parents('li#msg_186774').addClass('choice');
        var cfm=confirm('您确认要删除此条留言吗？');
        if(cfm==true){
            $.post('Blog/delreceive_msg',{'del':e.target.id},function(data){
                $('li#msg_186774').remove('.choice');
            },'json')
        }else if(cfm==false){
            $('li#msg_186774').removeClass('choice');
        }
    });
</script>
</body></html>